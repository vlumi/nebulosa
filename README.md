...
- Places: several pins, added with a double click or a long press on the map and named after the nearest place label the map is showing, dragged to move or locked against stray drags, renamed, reordered or removed in their sheet, kept in the browser. One is selected at a time, or none. The last row of the list places a pin at the browser's own location, only when asked: it is drawn as a target, cannot be dragged, moves only from the refresh on its row and carries a fixed label in the reader's language; the position stays in the browser like every other place.
- Passes over the selected place: the passes sheet lists every line-of-sight pass over the next 6 to 48 hours with rise, set and peak elevation, the time range's tooltip counting from the displayed moment, narrowed to the passes the radar can steer to or to the selected satellite. Show a pass to see where the satellite will be at its peak, or jump the clock to it.
