...
**Shaded surfaces.** The night side and the SAR reach band are MapLibre fill layers on GeoJSON sources, updated when the displayed minute changes: the basemap tessellates them onto its own globe, hides the far side itself, and shares no render state with anything. Its fills live in Mercator tiles, which end at 85°, so the two polar caps are drawn by deck.gl instead: the night there as cells (`nightCells`, each cut exactly along the terminator), the reach as the quads of the ribbon that pass 85°, both treated like the lines, lifted 30 km and depth-tested. The polar patch is small enough that the depth test's weakness at the limb never shows. An earlier round drew both surfaces entirely in deck.gl and spent days on what that costs: deck cuts globe polygons into 10° cells that sag into the basemap's sphere, no lift survives the limb, hiding the far side by hand lags the camera, and deck's render state leaks into MapLibre's cached GL state and makes it draw its own far-side tiles as dark wedges.
