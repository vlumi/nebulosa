...
**Resize.** The overlay drifts after a window resize, offset vertically by exactly the height change, and the fault is one branch in luma.gl's canvas context: luma keeps a framebuffer object for the canvas whose height sets the y-flip of every viewport drawn into it, and its deferred resize refreshes that object only when luma itself has to change the canvas size. MapLibre has already resized the canvas by then, so the object keeps the old height. On the map's resize event the map view therefore resizes that one object to the canvas, through private API named in one place in `MapView.tsx`; the fix is proposed upstream as visgl/luma.gl#3178 against issue #3177, and the handler goes when the lockfile's `@luma.gl/core` carries it.
