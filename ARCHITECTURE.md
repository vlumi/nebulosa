...
**Shaded surfaces.** The night side and the SAR reach band are deck.gl polygons built from small pieces: the night as strips three degrees of latitude tall, whose ends follow the terminator (`cos Δlon > tan lat · tan decl`, centered on the antisolar meridian) and which are rebuilt once per displayed minute; the reach as quads two track samples long. On the globe deck.gl cuts any polygon into 10° cells whose flat interiors sag some 24 km below the sphere, and a single big polygon speckled where those cells dipped into the basemap's own, coarser sphere. Small pieces sag little, and on the globe everything deck.gl draws floats 100 km above the ground, which clears the basemap's coarse polar mesh as well. MapLibre's own fill layers were tried and rejected: they live in Mercator tiles, which end at 85°, so neither surface could reach a pole.
