...
**Shaded surfaces.** The night side and the SAR reach band are deck.gl polygons built from small pieces: the night as cells three degrees square, each cut exactly where the terminator crosses it (the Sun is on the horizon where `tan lat = cos Δlon / tan decl`, Δlon measured from the antisolar meridian) and rebuilt once per displayed minute; the reach as quads two track samples long. Pieces stay small for three reasons: a big polygon sags into the basemap's sphere when deck.gl cuts it into 10° cells on the globe; anything wider than half the world is drawn inside out by that cutter; and each piece is judged visible on its own. The surfaces are drawn without a depth test, since at the limb no lift can separate them from the basemap's sphere, and the far side is left out instead by dropping pieces whose middle lies beyond the visible horizon, computed from MapLibre's camera geometry, about 77° from the view center at the default zoom. Lines and points keep a 30 km lift and the depth test, where it works. MapLibre's own fill layers were tried and rejected: they live in Mercator tiles, which end at 85°, so neither surface could reach a pole.

**Globe.** The map opens as a globe; `G` or the Globe pill flattens it to Mercator and back. MapLibre owns the projection, and the deck.gl overlay notices and swaps its own view to match, so tracks and picking carry over, and the far side is hidden by depth. Two adjustments make it clean: the text layer turns off back-face culling, because half the sphere's labels face away from the camera and would be dropped, and deck.gl's geometry floats 100 km above the ground in globe mode only, since a chord between two samples otherwise sinks in and out of the basemap's faceted sphere and reads as a dashed line, and the parallax at the limb stays under two percent of the radius; the lift is a model matrix applied after tessellation, because a third coordinate confuses the grid cutter. On the flat map the lift is zero, because a fixed altitude would displace things at street-level zoom, and deck.gl's depth test is off there, since nothing needs hiding and the test only made tracks z-fight with the ground at high zoom. The globe also yields to the flat map past zoom 5.5: curvature is invisible in a view that narrow, and the deck.gl MapLibre module, still a beta, clips its globe view away a little beyond that. The projection lives in the style, so it is applied when the style has loaded, on every toggle, and on every zoom.
