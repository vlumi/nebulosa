...
test('the ride needs a selected satellite; Esc closes the about, then leaves the ride, then closes the help', () => {
  const s = () => useApp.getState()
  s().setRide(true)
...
test('the help and the about panels share the corner, so opening one closes the other', () => {
  const s = () => useApp.getState()
  s().setHelpOpen(true)
  s().setAboutOpen(true)
  expect(s().helpOpen).toBe(false)
