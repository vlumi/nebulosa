...
test('shows no detail block until a satellite is selected', () => {
  render(<Panel satellites={sats} now={now} selected={null} onSelect={vi.fn()} span={{ pastOrbits: 1, futureOrbits: 1 }} onSpanChange={vi.fn()} />)
  expect(screen.queryByRole('definition')).toBeNull()
})

test('describes the selected satellite in human terms', () => {
  render(<Panel satellites={sats} now={now} selected={strix1.NORAD_CAT_ID} onSelect={vi.fn()} span={{ pastOrbits: 1, futureOrbits: 1 }} onSpanChange={vi.fn()} />)
...
test('the track span selects report a new span', async () => {
  ...
  render(<Panel satellites={sats} now={now} selected={null} onSelect={vi.fn()} span={{ pastOrbits: 1, futureOrbits: 1 }} onSpanChange={onSpanChange} />)
