...
test('coordinates are read from decimal, signed, lettered and DMS text, and not from anything else', () => {
  ...
  expect(parseLocation('n35.68 e139.77')).toEqual({ lat: 35.68, lon: 139.77 })
  expect(parseLocation('geo:60.17,24.94')).toEqual({ lat: 60.17, lon: 24.94 })
  expect(parseLocation('35 40.872 -139 46.026')).toEqual({ lat: 35.6812, lon: -139.7671 })
