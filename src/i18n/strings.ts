...
export const ja: Strings = {
  ...
  toggles: {
    ...
    reach: 'SAR',
    reachTitle: (min, max) => `選択中の衛星の軌跡からレーダーが届く地表: 直下から左右に${min}°〜${max}°`,
    language: '言語',
  },
  toolbar: {
    lists: '一覧',
  ...
  passes: {
    header: (place, min, max) =>
      `${place}上空で地平線より上に見える通過です。ピンはドラッグで動かせます。強調表示のピークはレーダーが向けられる直下${min}〜${max}°の範囲内です。`,
    next: '今後',
    hoursAhead: '先の時間',
    hours: (h) => `${h}時間`,
    show: '表示',
    scope: 'パス',
    scopeTitle: '見えるパスすべてか、レーダーが向けられるものだけか。真上は側視レーダーには近すぎます',
    aboveHorizon: '地平線上すべて',
    inReach: 'SAR撮像範囲内',
    only: (name) => `${name}のみ`,
    none: 'ありません。',
    showTitle: 'ピーク時の衛星の位置を表示',
    offNadir: (deg, inReach) => `ピーク時の直下角${deg}°${inReach ? '、SAR撮像範囲内' : ''}`,
    goTo: (name, at) => `${at}の${name}のパスへ移動`,
  },
  time: {
    live: 'ライブ',
    pause: '一時停止',
